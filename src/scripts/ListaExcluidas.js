...
var tarefas = usuarioCorrente.excluidas;
...
function preencherLista(query, sort, tipo) {
    ...
    listaHtml.innerHTML = "";
    tarefas = usuarioCorrente.excluidas;
...
function abrirModalGenerico(id) {
    let tarefas = usuarioCorrente.excluidas;
    let modal = ``;
    for (let i = 0; i < tarefas.length; i++) {
        if (tarefas[i].id == id) {
            modal = `
                <div id="modal" class="modalCriacao">
                    <div id="corpoModal">
                        ...
                        <div id="divBotoes">
                            <button class="btn btnArquivar" id="btnArquivar">Restaurar</button>
...
function restaurarTarefa(id) {
    let excluidas = usuarioCorrente.excluidas;
    var novaArquivada = {};
    for(let i = 0; i < excluidas.length; i++){
        if(excluidas[i].id == id){
            var index = excluidas.indexOf(excluidas[i]);
            novaArquivada = {
                idUsuario: excluidas[i].idUsuario,
                id: excluidas[i].id,
                checked: excluidas[i].checked,
                nome: excluidas[i].nome,
                desc: excluidas[i].desc,
                categoria: excluidas[i].categoria,
                prioridade: excluidas[i].prioridade,
                dataadd: excluidas[i].dataadd,
                datavenc: excluidas[i].datavenc
            }
            excluidas.splice(index, 1);
        }
    }
    fecharModal();
